...
import (
	...
	pb "github.com/devansh-akruvala/task-scheduler/pkg/grpcapi"
...
func (s *CoordinatorServer) Start() error {
	...
	if err = s.startGrcpServer(); err != nil {
		return fmt.Errorf("gRCP server failed to start: %w", err)
	}
...
func (s *CoordinatorServer) startGrcpServer() error {
	var err error
	s.listener, err = net.Listen("tcp", s.serverPort)
	if err != nil {
		return err
	}
	log.Printf("Starting gRCP server on %s", s.serverPort)
	s.grpcServer = grpc.NewServer()
	pb.RegisterCoordinatorServiceServer(s.grpcServer, s)

	go func() {
		if err := s.grpcServer.Serve(s.listener); err != nil {
			log.Fatalf("gRCP server failed: %v", err)
		}
	}()
...
func (s *CoordinatorServer) getNextServer() *workerInfo {
	s.WorkerPoolKeysMutex.RLock()
	defer s.WorkerPoolKeysMutex.RLock()
...
func (s *CoordinatorServer) executeAllScheduledTasks() {
	...
	if err := tx.Commit(ctx); err != nil {
		log.Printf("Failed to commit tranx :%v\n", err)
	}
}
